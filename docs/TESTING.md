...
| `test_telemetry.py`, `test_build_provenance.py`, `test_stage_budgets.py` | Stage timings, FFmpeg/runtime identity, per-stage deadlines |
| `test_golden_fixtures.py` | Generated people/face/OCR/speech/VFR/rotation clips |
| `test_ppocr_parity.py`, `test_ppocr_recall.py`, `test_export_ppocr.py` | Box IoU, Paddle/ONNX parity rules, synthetic banner ROI, export lock pins |
...
| `test_release_gates.py` | Telemetry, FFmpeg provenance, decoded-pixel and stage budgets, VFR/rotation/speech goldens |
...
CI runs the default suite on Linux x86_64 and a separate `real-models-linux`
job that vendors weights and executes `-m real_models`. Apple Silicon may run
the same commands as a smoke test; do not treat Mac CFR hashes as production
parity.
...
4. Never commit a developer `Downloads` fixture. Generate media with ffmpeg in
   `tmp_path` via `tests/fixtures/generate.py`, or skip until a SHA-pinned
   CC0 object exists.
...
- Owned PP-OCR ONNX hosted and pinned; Paddle/ONNX `reference.npz` not recorded yet
- Real YOLOX / YuNet recall thresholds on people or faces
- ByteTrack ID-switch rate or cut-reset on a multi-person fixture
- Rights-cleared CC0 people/face/speech reels pinned by SHA-256
...
- Real speech recall/precision for Silero on a labeled corpus
...
- Published p50/p95 CPU/RSS on a 60s 1080p reel (harness exists, numbers do not)
...
| `test_prod_real_http_pipeline.py` | Real model boot + all-v1 `/analyze` smoke |
| `test_prod_real_golden_fixtures.py` | Real models on generated OCR, speech, VFR, and rotation clips |
| `test_prod_ppocr_parity.py` | Owned-export gate requires a committed Paddle reference tensor |
| `test_prod_ppocr_recall.py` | Real ONNX recall/precision on a synthetic top-banner Latin clip |
...
real_models   downloads real ONNX weights (required on Linux CI; excluded from default pytest)
```
